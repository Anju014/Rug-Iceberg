...
function draw() {
  ...
  for (var i = 0; i < icebergsGroup.length; i++) {
    if (fireballGroup.isTouching(icebergsGroup[i])) {
      icebergsGroup[i].destroy()
...
function shootFire() {
  fireball = createSprite(ship.x+60, ship.y-40)
...
function spawnicebergs() {
  if (frameCount % 100 === 0) {
    iceberg = createSprite(1000, height - 230)
